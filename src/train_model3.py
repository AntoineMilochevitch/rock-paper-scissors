...
def build_resnet_model(num_classes: int) -> nn.Module:
    ...
    if FREEZE_BACKBONE:
        for name, param in model.named_parameters():
            if any(name.startswith(layer_name) for layer_name in FINE_TUNE_LAYERS):
...
def build_dataloader(dataset: datasets.ImageFolder, shuffle: bool) -> DataLoader:
...
def train_model() -> None:
    ...
    raw_train = datasets.ImageFolder(DATA_ROOT / "train", transform=raw_train_transform)

    train_dataset = datasets.ImageFolder(DATA_ROOT / "train", transform=build_train_transform(TARGET_SIZE))
    validation_dataset = datasets.ImageFolder(DATA_ROOT / "validation", transform=build_eval_transform(TARGET_SIZE))
    test_dataset = datasets.ImageFolder(DATA_ROOT / "test", transform=build_eval_transform(TARGET_SIZE))
    ...
    model = build_resnet_model(num_classes=len(class_names)).to(DEVICE)
    ...
    run_name = f"model3_resnet_{run_timestamp}"
    ...
    torch.save(
        {
            ...
            "config": {
                ...
                "fine_tune_layers": list(FINE_TUNE_LAYERS),
                "device": str(DEVICE),
            },
            "best_epoch": best_epoch,
        },
